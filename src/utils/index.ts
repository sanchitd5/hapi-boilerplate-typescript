...
import { AuthType, RouteProperties, FrozenResponseMessage, GenericObject } from '../definations';
import Joi from "joi";
import MD5 from "md5";
...
export const sendError = (data: GenericObject | string | boolean | Error | FrozenResponseMessage | unknown) => {
    console.trace('ERROR OCCURED ', data)
    if (converters.toObject(data)) {
        if (typeof data == 'object' && data.hasOwnProperty('statusCode') && data.hasOwnProperty('customMessage') && converters.toFrozenResponseMessage(data)) {
...
export const sendSuccess = (successMsg: GenericObject | string | undefined = CONFIG.APP_CONSTANTS.STATUS_MSG.SUCCESS.DEFAULT.customMessage, data: GenericObject | Array<GenericObject> | unknown) => {
...
export const generateFilenameWithExtension = function generateFilenameWithExtension(oldFilename: string, newFilename: string) {
...
export const getRange = (startDate: Date, endDate: Date, diffIn: any = CONFIG.APP_CONSTANTS.TIME_UNITS.HOURS) =>
 ...
 */
export const cleanObject = (obj: GenericObject, callback?: (param: GenericObject) => GenericObject): object => {
