				...
				(err) => {
					if (err) callback(err);
						...
						callback(null, {
							...
							userDetails: this.utils.deleteUnnecessaryUserData(
								customerData
							),
		...
		this.async.series(
					...
					if (!userFound) cb(this.ERROR.USER_NOT_FOUND as any);
						...
						if (userFound.isBlocked) cb(this.ERROR.ACCOUNT_BLOCKED as any);
							...
							if (
								userFound &&
								userFound.password !=
								this.utils.CryptData(payloadData.password)
							) {
			...
			(err) => {
				if (err) callback(err);
					...
					callback(null, {
						accessToken: accessToken,
						userDetails: this.utils.deleteUnnecessaryUserData(updatedUserDetails),
						appVersion: appVersion
					});
				}
			}
